...
public class AnalySynth {
	...
	public static void rSearch(ArrayList<String> list, String givenDifferences, ArrayList<String> reactants, int n) throws Exception {
		helper(list, new ArrayList<String>(), new ArrayList<String>(), reactants, givenDifferences, n);
	}
	
	public static void helper(ArrayList<String> a, ArrayList<String> b, ArrayList<String> c, ArrayList<String> d, String givenDifferences, int n) throws Exception {
		long time = System.currentTimeMillis();
		for(int i = 0; i < a.size() && n >= 0; i++) {
			String temp = a.get(i).substring(a.get(i).indexOf(" ")+1);
			b.add(a.get(i).substring(0, a.get(i).indexOf(" ")));
			if(!helper2(b, d) || (c.size() > 0 && !c.get(c.size()-1).equals(temp) && c.contains(temp))) {
				b.remove(b.size()-1);
				continue;
			}
			c.add(temp);
			if(retroSynth(b).equals(givenDifferences))
				System.out.println(c);
			helper(a, b, c, d, givenDifferences, n-1);
			if(b.size() > 0) {
				b.remove(b.size()-1);
				c.remove(c.size()-1);
			}
	...
	public static boolean helper2 (ArrayList<String> reaction, ArrayList<String> reactants) throws Exception {
		
		for(int i = 0; i < reaction.size(); i++)
			if(reaction.get(i).substring(0, 1).equals("["))
				reaction.set(i, reaction.get(i).substring(1, reaction.get(i).length()-1));
		ArrayList<ArrayList<String>> rxns = new ArrayList<ArrayList<String>>();
		for(int i = 0; i < reaction.size(); i++) {
			ArrayList<String> rxn = new ArrayList<String>();
			String r = reaction.get(i);
			while(r.contains("]]")) {
				rxn.add(r.substring(0, r.indexOf("]]")+2));
				r = r.substring(r.indexOf("]]")+2);
			}
			Collections.sort(rxn);
			rxns.add(rxn);
		}
		ArrayList<String> r = new ArrayList<String>();
		for(int i = 0; i < reactants.size(); i++) {
			ArrayList<String> subR = InchiArray(reactants.get(i), 2, true);
			for(int j = 0; j < subR.size(); j++)
				subR.set(j, subR.get(j).replaceAll(", ", ""));
		...
		for(int i = 0; i < rxns.size(); i++)
			for(int j = 0; j < rxns.get(i).size(); j++)
				if(rxns.get(i).get(j).substring(0, 1).equals("-")) {
					if(!r.contains(rxns.get(i).get(j).substring(1))) {
						//times.add(System.currentTimeMillis()-time);
						return false;
					}
					else {
						for(int k = 0; k < r.size(); k++)
							if(r.get(k).equals(rxns.get(i).get(j).substring(1))) {
								r.remove(k);
								rxns.get(i).remove(j);
								j--;
								break;
	...
	public static void justATest() {
		...
		for(int i = 1; i <= 10; i++) {
			System.out.print(times.get(times.size()-i) + " ");
		}
		System.out.println();
