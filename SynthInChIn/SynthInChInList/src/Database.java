...
public class Database extends AnalySynth{
	...
	public static void databaseDiff(File database1, File database2, int complex, boolean bt) throws Exception {
		...
		while(in.ready()) {
			...
			p.close();
			if(!Reactants.isEmpty() && !Products.isEmpty() && Reactants.size() < 10 && Products.size() < 10) {
				...
				for(int i = 0; i < rPerm.size(); i++)
					for(int j = 0; j < pPerm.size(); j++)
						print.println(differences(rPerm.get(i), pPerm.get(j), complex, bt) + c);
			}
			else if(!Reactants.isEmpty() && !Products.isEmpty()) {
				if(Products.get(Products.size()-1).contains("|"));{
					try {
						String temp = Products.get(Products.size()-1);
						Products.set(Products.size()-1, temp.substring(0, temp.indexOf("|")));
				...
				print.println(differences(Reactants, Products, complex, bt) + c);
				for(int i = 0; i < Reactants.size(); i++)
					for(int j = 0; j < Products.size(); j++)
						print.println(differences(new ArrayList<String>(Arrays.asList(Reactants.get(i))), new ArrayList<String>(Arrays.asList(Products.get(j))), complex, bt) + c);
			}
		}
		in.close();
		print.close();
